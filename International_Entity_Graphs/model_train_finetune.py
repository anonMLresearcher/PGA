...
#     m = len(sim_list)
    ...
    vec_list = []
    for num_1 in range(2,6):
        vec = gen_pos_vectors_max_inter_angle(6,num_1)
        vec_list.append(vec)
    ...
    pref_vecs_hard = tf.eye(6)
    pref_vecs_hard = tf.concat([pref_vecs_hard,tf.ones((1,6))/6.],0)
    ...
    pref_vecs = tf.concat([pref_vecs_prior,tf.random_uniform((20,6))],0)
    ...
    for i,sim in enumerate(sim_list):
        ...
        if i == 0:
            ...
            # nation_sp = ctx.get_config("nation")
            dic_nation2exp = {'TH':1,'VN':3,'SG':3,'MY':2,'ID':1,'PH':2}
