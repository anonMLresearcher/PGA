...
gm_loss_weight=1,flag_dist=1.,var_weight=1e-6,
gate_weight=1.,gate_th=0.8,flag_gate_above=True,
    ...
    for i_level in range(len(x)):
        ...
        for i_nation in range(len(e_x)):
            ...
            for j_nation in range(len(e_x)):
                ...
                dist_pos = tf.reduce_mean(gm_dist(e_x_i,e_y_j)*e_x_i_g[:,:,j_nation]*e_y_j_g[:,:,i_nation])
                dist_neg = tf.reduce_mean(gm_dist(e_x_i,e_yr_j)*e_x_i_g[:,:,j_nation]*e_yr_j_g[:,:,i_nation])
        ...
        loss_num = loss_num + dist_sum/cont + gate_weight*loss_gate/cont + var_weight * flag_dist* loss_var_sum/cont
...
def sgwd_loss(x,y,nation_exist,gwd_idx,gwd_idx_norm,x_g,y_g,ps_num, init_val, 
                stddev,mode='gwd',nation_weights=None,gm_loss_weight=1,flag_dist=1.,
                var_weight=1e-6,gate_weight=1.,gate_th=0.8,
                flag_gate_above=True,same_nation_weight=1.,
    ...
    for i_level in range(len(x)):
        ...
        for i_nation in range(len(e_x)):
            ...
            for j_nation in range(len(e_x)):
                ...
                dist_pos = tf.reduce_mean(gm_dist(dist_i,dist_j)*gate_i[:,:,j_nation]*gate_j[:,:,i_nation])
                dist_neg = tf.reduce_mean(gm_dist(dist_i,dist_r_j)*gate_i[:,:,j_nation]*gate_r_j[:,:,i_nation])
                ...
                loss_gate = loss_gate +  ( (1.-flag) * loss_gate_pos +  flag * loss_gate_neg ) * nation_weights[j_nation] * pair_nation_weight
        ...
        loss_num = loss_num + 2.* ( dist_sum/cont + gate_weight*loss_gate/cont + var_weight * flag_dist * loss_var_sum )
