...
def mapping_net(id_deep, ps_num, init_val, stddev, sp_name,trainable=False):
    with tf.variable_scope("mapping_layer1_%s"%sp_name):
        x = full_connect(id_deep, [32, 32], [32], False, ps_num, init_val, stddev,trainable=trainable)
        x = tf.nn.elu(x)
...
def swd_proj_src_with_idx(x,idx_list, nation_idx_where,flag_colwise=False,slice_dim=128):
    ...
    if flag_colwise:
        for i_idx,e_idx in enumerate(nation_idx_where):
            ...
            idx_sort = idx_list[i_idx]

            e_x = tf.stack([tf.gather(e_x_i[:,i_col],idx_sort[:,i_col]) for i_col in range(slice_dim)],axis=1)
...
def swd_proj_src_with_idx_RI(xx,idx_list, nation_idx_where,flag_colwise=False,slice_dim=128):
...
def swd_proj_pos_neg_with_idx(pos,neg, idx_list, nation_idx_where,neg_num,flag_colwise=False,slice_dim=128):
...
def swd_proj_pos_neg_with_idx_RI(pos_list,neg_list, idx_list, nation_idx_where,neg_num,flag_colwise=False,slice_dim=128):
...
def mapping_fun(current_id_ays, ps_num, dense_ini_val, dense_stddev,nation_list,Theta=None,trainable=False):
    current_id_ays_nation_list = []
    for e_nation in nation_list:
        current_id_ays_nation=mapping_net(current_id_ays, ps_num, dense_ini_val, dense_stddev, e_nation,trainable=trainable)
        if Theta is not None:
            current_id_ays_nation = tf.matmul(current_id_ays_nation,Theta, transpose_b=True)
