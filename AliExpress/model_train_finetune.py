...
dic_nation2exploss = {'ES':1,'FR':1,'NL':1,'RU':1,'US':1}
...
print('align_weight',align_weight)
...
def dense_embedding(sp_tensors, input_dimensions, embedding_dimension, ps_num, init_val, stddev, names):
    ...
    print('len(names)',len(names))
    for i in range(len(sp_tensors)):
        with tf.variable_scope('main_'+names[i]):
...
def sample_by_walk_direct(src, edge_type, walk_len=1, walk_p=1, walk_q=1):
...
def sample_pos_n_feature(source,dst_id,full_features,q_part_features,i_part_features,c_part_features,full_features_dense,full_feature_dims_dense):
    node_list = []
    src  = sample_by_walk_direct(source, ['3'])
    pos  = sample_by_walk_direct(dst_id, ['3'])
    neg = tf.concat([pos,pos,pos,pos,pos,pos],0)
    ...
    pos_dense = tf_euler.get_dense_feature(pos, full_features_dense, full_feature_dims_dense)
    neg_dense = tf_euler.get_dense_feature(neg, full_features_dense, full_feature_dims_dense)
    ...
    return feature_dic,src_dense, pos_dense, neg_dense
...
def inference(tensors, keep_prob=1.0, ps_num=1):
    ...
    dst_id=tensors.data[4][0].values
    # negs = tensors.data[1][0].values

    dst_id = get_high64bit(dst_id)
    # negs = get_high64bit(negs)

    label_vec = tf.to_float(tensors.data[5])
    ...
    dic_nation2int={'ES':1,'FR':2,'NL':3,'RU':4,'US':5}
...
#     Theta_gwd=tf.nn.l2_normalize(Theta_gwd,axis=1)
    ...
    feature_dic,src_dense, pos_dense, neg_dense = sample_pos_n_feature(source,dst_id,full_feature_names,part_q_feature_names,part_i_feature_names,part_c_feature_names,full_feature_names_dense,full_feature_dims_dense)
    full_node_dense = [src_dense, pos_dense, neg_dense]
    ...
    with tf.variable_scope("embedding", reuse=tf.AUTO_REUSE) as scope:
        ...
        for i_part,e_part in enumerate(feature_dic):
            e_list = feature_dic[e_part]
            for i_level,e in enumerate(e_list):
                dic={}
                for i_type,e_type in enumerate(e):
                    ...
                    if e_part == 'self':
                        full_node_embed_dense = dense_embedding(full_node_dense[i_type], full_feature_dims_dense,
                                                full_emb_dim_dense,
                                                ps_num,
...
#             x,xr=SWD_proj(pos_id_ays, Theta,nation_idx_where)
    ...
    with tf.variable_scope("finetune_att_sim", reuse=tf.AUTO_REUSE) as scope:
        ...
        for i_level in range(n_level):
            if i_level == 0:
                ...
                pos_id_ays = pos_id_ays_list[i_level]
                att_sim_con = tf.reduce_sum(current_id_ays*pos_id_ays,1,keepdims=True)
    ...
    return att_sim_list,nation_onehot,align_info,label_vec
...
#     m = len(sim_list)
    ...
    sim_list,nation_onehot,align_info,label_vec  = input_list
    ...
    vec_list = []
    for num_1 in range(2,n_nation):
        vec = gen_pos_vectors_max_inter_angle(n_nation,num_1)
        vec_list.append(vec)
    ...
    for i,sim in enumerate(sim_list):
        ...
        if i==0:
            label = tf.reshape(label_vec, [-1, 1])
            sim = tf.reshape(sim, [-1, 1])
            loss = 7*tf.nn.weighted_cross_entropy_with_logits(targets=label, logits=gama*sim, pos_weight=2.0)
        ...
        if i == 0:
            ...
            # nation_sp = ctx.get_config("nation")
            dic_nation2exp = {'ES':1,'FR':1,'NL':2,'RU':3,'US':2}
            ...
            #     flag_global_step = greater_than_zero(global_step-1280000.)
...
#             loss_param_regu = cont_loss* weight_decay * tf.add_n(reg_losses)
    ...
    loss_sum = loss_sum + align_weight * loss_regu
          

    var_list = get_variable_list('gate')
         
    return loss_sum/7., [loss,loss_param_regu,loss_regu,loss_vec,flag,weight_init,weight_soft,weight,loss_vec2,
                     show_list_swd,
                     show_list_sgwd,
                     show_list_scgwd] 
...
def auc(input_list, num_thresholds=200, decay_rate=1):
    ...
    sim=sim_list[0]
    labels = tf.reshape(label_vec, [-1, 1])
